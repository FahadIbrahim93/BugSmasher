...
import { signInWithProvider, signOut, googleProvider, githubProvider, discordProvider, User } from '../firebase';
...
>>>>>>> 6ff639e (feat: transform BugSmasher into AAA-quality game)
  ...
  const handleLogin = async (providerName: 'google' | 'github' | 'discord') => {
    ...
    try {
      ...
      await signInWithProvider(providers[providerName]);
      onAuthChange?.();
    } catch (err: any) {
    ...
    <div id="auth-section" style={{ marginTop: '40px', display: 'flex', flexDirection: 'column', alignItems: 'center', zIndex: 2 }}>
      {!user ? (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px', width: '100%', maxWidth: '300px' }}>
          <div style={{ fontSize: '14px', color: 'rgba(100, 200, 255, 0.7)', fontFamily: "'Orbitron', monospace", letterSpacing: '1px', marginBottom: '10px' }}>
            AUTHENTICATE
          </div>
          <button 
            className="start-btn" 
...
>>>>>>> 6ff639e (feat: transform BugSmasher into AAA-quality game)
        ...
        <div id="user-info" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '10px' }}>
          ...
          <button 
            id="logout-btn" 
            style={{ background: 'transparent', border: 'none', color: 'rgba(255,100,100,0.8)', fontFamily: "'Orbitron', monospace", fontSize: '10px', cursor: 'pointer', letterSpacing: '2px', textDecoration: 'underline', marginTop: '10px' }} 
