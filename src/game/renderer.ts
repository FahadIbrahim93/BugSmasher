...
export class InsectRenderer {
  static drawBeetle(
    ...
    cx.scale(1, 0.3);
    cx.beginPath();
    cx.ellipse(0, s * 2, s * 1.2, s * 0.8, 0, 0, TAU);
    cx.fillStyle = rgb(0, 0, 0, 0.25);
    cx.fill();
    cx.restore();
    ...
    cx.beginPath();
    cx.ellipse(-s * 0.3, 0, s * 0.55, s * 0.4, 0, 0, TAU);
    cx.fillStyle = abdG;
    cx.fill();
    ...
    cx.beginPath();
    cx.ellipse(s * 0.15, 0, s * 0.3, s * 0.3, 0, 0, TAU);
    cx.fillStyle = thorG;
    cx.fill();
    ...
    cx.beginPath();
    cx.ellipse(s * 0.5, 0, s * 0.22, s * 0.2, 0, 0, TAU);
    cx.fillStyle = headG;
    cx.fill();

    for (let side = -1; side <= 1; side += 2) {
      cx.beginPath();
      cx.ellipse(s * 0.6, side * s * 0.1, s * 0.08, s * 0.09, 0, 0, TAU);
      cx.fillStyle = palette.eye;
      cx.fill();
      cx.beginPath();
      cx.arc(s * 0.62, side * s * 0.08, s * 0.03, 0, TAU);
    ...
    for (let side = -1; side <= 1; side += 2) {
      ...
      cx.quadraticCurveTo(
        s * 0.8 + Math.cos(antWave) * s * 0.1,
        side * s * 0.3,
        s * 0.9,
        side * (s * 0.35 + Math.sin(antWave) * s * 0.1)
      );
      ...
      cx.stroke();
      cx.beginPath();
      cx.arc(s * 0.9, side * (s * 0.35 + Math.sin(antWave) * s * 0.1), s * 0.03, 0, TAU);
      cx.fillStyle = palette.antenna;
      cx.fill();
    }

    cx.beginPath();
    cx.ellipse(s * 0.15, 0, s * 0.3, s * 0.3, 0, -0.5, 0.5);
    ...
    if (type === 'wasp' || type === 'moth') {
      ...
      for (let side = -1; side <= 1; side += 2) {
        ...
        cx.ellipse(
          ...
          s * 0.5,
          s * 0.2,
          side * 0.2 + Math.sin(animPhase * 15) * 0.3,
          0,
          TAU
        );
