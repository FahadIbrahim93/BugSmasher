...
export class Particle {
  ...
  draw(cx: CanvasRenderingContext2D) {
    ...
    if (this.shape === 'circle') {
      cx.beginPath();
      cx.arc(0, 0, this.currentSize, 0, TAU);
...
export function spawnParticles(x: number, y: number, count: number, config: any = {}) {
  for (let i = 0; i < count; i++) {
    const a = rand(0, TAU),
      spd = rand(config.minSpeed || 1, config.maxSpeed || 4);
    particles.push(
      new Particle(x + rand(-5, 5), y + rand(-5, 5), {
        ...
        color: config.colors
          ? config.colors[randInt(0, config.colors.length - 1)]
          : config.color || '#00ff64',
